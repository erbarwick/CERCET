...
class PeMSHandler(object):
    ...
    def download_files(self, start_year, end_year, districts, file_types, months, save_path=None):
        ...
        if not files_to_download.empty:
            for index, row in files_to_download.iterrows():
                ...
                if success:
                    # append() no longer supported. Replaced with currently supported code.
                    files_downloaded = pd.concat([files_downloaded, row.to_frame().T], ignore_index=True)
                time.sleep(10)
    ...
    def _browser_login(self):
        ...
        browser.set_handle_refresh(mechanize._http.HTTPRefreshProcessor(), max_time=30)
    ...
    def _parse_html(self):
        ...
        script = self.html_object.find('script', text=re.compile(r'YAHOO\.bts\.Data'))
